...
int m_;
for( m_=1;(m_<=produc_[i].size())&&(produc_[i][m_]<N)&&(index>=0);m_++){
...
/******************FOLLOW 集合构造*************/
void
follow(vector<vector<int> >&follow_,vector<vector<int> >produc_,vector<vector<int> >first_,int size,int null_index){
int tag=1;
int index;
bool bl_;
index=size+N;
...
/****************预测分析表构造*************/
void
table(vector<vector<int> >&table_,vector<vector<int> >produc_,vector<vector<int> >first_,vector<vector<int> >&follow_,int null_index){
for(int i=0;i<produc_.size();i++){
int m=produc_[i][0];
int n=produc_[i][1];
if(n==null_index)
...
/*非递归预测分析算法*/
void
pred_anal(vector<vector<int> >table_,vector<vector<int> >produc_,vector<string>termi_,vector<string>un_ter_,string s){
vector<int>temp;
stack<int> st;
string s1;
